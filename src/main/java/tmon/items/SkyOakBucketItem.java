...
public class SkyOakBucketItem extends Item {
	protected final Fluid fluid;

	public SkyOakBucketItem(Fluid fluid, Item.Settings settings) {
		super(settings);
		this.fluid = fluid;
	}
				...
				if (this.fluid == Fluids.EMPTY) {
					...
					if (state.getBlock() instanceof FluidDrainable) {
						...
						if (fluid == Fluids.WATER) {
							...
							player.playSound(fluid.matches(FluidTags.LAVA) ? SoundEvents.ITEM_BUCKET_FILL_LAVA : SoundEvents.ITEM_BUCKET_FILL, 1.0F, 1.0F);
							ItemStack filled = this.getFilledStack(stack, player, TMoN.SKY_OAK_WATER_BUCKET);
							if (!world.isClient) {
								Criterions.FILLED_BUCKET.handle((ServerPlayerEntity) player, new ItemStack(fluid.getBucketItem()));
							}
					...
					if (this.placeFluid(player, world, pos2, blockHitResult)) {
						...
						if (player instanceof ServerPlayerEntity) {
							Criterions.PLACED_BLOCK.handle((ServerPlayerEntity) player, pos2, stack);
						}
	...
	protected ItemStack getEmptiedStack(ItemStack stack, PlayerEntity player) {
		return !player.abilities.creativeMode ? new ItemStack(TMoN.SKY_OAK_BUCKET) : stack;
	}
	...
	protected void playEmptyingSound(PlayerEntity player, IWorld world, BlockPos pos) {
		SoundEvent sound = this.fluid.matches(FluidTags.LAVA) ? SoundEvents.ITEM_BUCKET_EMPTY_LAVA : SoundEvents.ITEM_BUCKET_EMPTY;
